...
def test_get_E_log_p_x_z():
    ...
    x_params = [
        np.random.normal(2,1,size=(60,2)).astype(np.float32),
        np.random.normal(2,1,size=(60,1)).astype(np.float32),
        tf.nn.softmax(np.random.normal(2,1,size=(60,2)).astype(np.float32)).numpy()]
    assert(np.max(
        tf.reduce_sum(x_params[2], axis=-1).numpy() 
            - np.ones((8,1)) < 1e-3))
    trivial_miss_list = np.ones(tf.shape(x)).astype(np.float32)
    E_log_p_x_z = hvf.get_E_log_p_x_z(z_shape, x_params, x,
        column_types,trivial_miss_list)
    assert(E_log_p_x_z.numpy().shape == (s_dim, ))

def test_get_E_log_q_z_x():
    z_samples = np.random.normal(2,1,(5,4,3,2)).astype(np.float32)
    mu_z = np.random.normal(2,0.1,(3,2)).astype(np.float32)
    log_sigma_z = np.random.normal(3,0.5,(3,2)).astype(np.float32)
    E_log_q_z_x = hvf.get_E_log_q_z_x(z_samples, mu_z, log_sigma_z)
    assert(E_log_q_z_x.numpy().shape == (5,))